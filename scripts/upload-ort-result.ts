...
import type {OrtAnalyzerInput} from 'ortcompressor';
...
interface HorusecConfig {
    horusecCliRepositoryName?: string;
    [key: string]: unknown;
}

function fail(message: string, extra: string | number = ''): never {
...
function resolveHorusecConfig(projectRoot: string): string | null {
    const candidates = [
...
function resolveProjectName(projectRoot: string, configPath: string | null): string {
    const fallback = path.basename(projectRoot);

    if (!configPath) return fallback;

    const config = readJsonFile<HorusecConfig>(configPath);
    const projectName = config.horusecCliRepositoryName ?? fallback;

    return String(projectName).trim() || fallback;
}

function renderPrComment(projectName: string, violations: Violation[]): string {
...
async function main(): Promise<void> {
    ...
    const input = JSON.parse(fs.readFileSync(analyzerResultPath, 'utf-8')) as OrtAnalyzerInput;
    ...
    if (response.status === 417) {
        try {
            violations = (await response.json()) as Violation[];
        } catch {
